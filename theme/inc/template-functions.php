 ...
 * @return array
 */
function hefty_starter_theme_body_classes( $classes ) {
...
add_filter( 'body_class', 'hefty_starter_theme_body_classes' );
 ...
 * @return array
 */
function hefty_starter_theme_post_classes( $classes ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'hefty_starter_theme_post_classes', 10, 3 );
 ...
 * @return void
 */
function hefty_starter_theme_supports_js() {
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action( 'wp_footer', 'hefty_starter_theme_supports_js' );
 ...
 * @return array
 */
function hefty_starter_theme_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'hefty_starter_theme_comment_form_defaults' );
 ...
 * @return bool
 */
function hefty_starter_theme_can_show_post_thumbnail() {
	/**
	...
	return apply_filters(
		'hefty_starter_theme_can_show_post_thumbnail',
 ...
 * @return int
 */
function hefty_starter_theme_get_avatar_size() {
	return 60;
}
 ...
 * @since 1.0.0
 */
function hefty_starter_theme_continue_reading_text() {
	$continue_reading = sprintf(
		/* translators: %s: Name of current post. */
		esc_html__( 'Continue reading %s', 'hefty-starter-theme' ),
 ...
 * @since 1.0.0
 */
function hefty_starter_theme_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . hefty_starter_theme_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'hefty_starter_theme_continue_reading_link_excerpt' );
 ...
 * @since 1.0.0
 */
function hefty_starter_theme_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . hefty_starter_theme_continue_reading_text() . '</a></div>';
	}
}

// Filter the excerpt more link.
add_filter( 'the_content_more_link', 'hefty_starter_theme_continue_reading_link' );

if ( ! function_exists( 'hefty_starter_theme_post_title' ) ) {
	/**
	 ...
	 * @return string
	 */
	function hefty_starter_theme_post_title( $title ) {
		return '' === $title ? esc_html_x( 'Untitled', 'Added to posts and pages that are missing titles', 'hefty-starter-theme' ) : $title;
	}
}
add_filter( 'the_title', 'hefty_starter_theme_post_title' );
 ...
 * @return string
 */
function hefty_starter_theme_get_icon_svg( $group, $icon, $size = 24 ) {
	return hefty_starter_theme_SVG_Icons::get_svg( $group, $icon, $size );
}
 ...
 * @return string
 */
function hefty_starter_theme_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? hefty_starter_theme_get_icon_svg( 'ui', 'arrow_right' ) : hefty_starter_theme_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? hefty_starter_theme_get_icon_svg( 'ui', 'arrow_left' ) : hefty_starter_theme_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'hefty_starter_theme_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function hefty_starter_theme_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'hefty_starter_theme_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'hefty_starter_theme_get_localized_font_family_elements',
		array(
	...
	if ( ! function_exists( 'hefty_starter_theme_generate_css' ) ) {
	...
	return hefty_starter_theme_generate_css( // @phpstan-ignore-line.
 ...
 */
function hefty_starter_theme_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 */
function hefty_starter_theme_password_form( $output, $post = 0 ) {
	$post   = get_post( $post );
	$label  = 'pwbox-' . ( empty( $post->ID ) ? wp_rand() : $post->ID );
	$output = '<p class="post-password-message">' . esc_html__( 'This content is password protected. Please enter a password to view.', 'hefty-starter-theme' ) . '</p>
	<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
	<label class="post-password-form__label" for="' . esc_attr( $label ) . '">' . esc_html_x( 'Password', 'Post password form', 'hefty-starter-theme' ) . '</label><input class="post-password-form__input" name="post_password" id="' . esc_attr( $label ) . '" type="password" size="20" /><input type="submit" class="post-password-form__submit" name="' . esc_attr_x( 'Submit', 'Post password form', 'hefty-starter-theme' ) . '" value="' . esc_attr_x( 'Enter', 'Post password form', 'hefty-starter-theme' ) . '" /></form>
	';
	return $output;
}
add_filter( 'the_password_form', 'hefty_starter_theme_password_form', 10, 2 );
 ...
 * @return array
 */
function hefty_starter_theme_get_attachment_image_attributes( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'hefty_starter_theme_get_attachment_image_attributes', 10, 3 );
